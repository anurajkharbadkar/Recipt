...
export default function GoogleAuthButton({ onSuccess, onUnregistered, text }: GoogleAuthButtonProps) {
  ...
  useEffect(() => {
    ...
    const initGoogleGis = () => {
      if (window.google?.accounts?.id) {
        ...
        // Render Google's native standard button into the container if element exists
  ...
  return (
    <div className="w-full flex flex-col items-center gap-2">
      {/* Container for Google Native GIS Button */}
      {googleClientId && (
        <div id="google-btn-container" className="w-full flex justify-center min-h-[44px]" />
      )}

      {/* Fallback / Custom styled button if native button hasn't rendered yet or client ID is pending */}
